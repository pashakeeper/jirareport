$(document).ready(function() {
    ...
    function initSwipers() {
        ...
        window._swipers.logos = new Swiper(".logos-slider", {
            speed: 4000,
            autoplay: { delay: 1, disableOnInteraction: false }, // iOS: не 0!
            loop: true,
            slidesPerView: 'auto',
    ...
    function whenImagesLoaded($root, cb) {
        ...
        $imgs.each(function() {
            if (this.complete) { if (--left === 0) cb(); } else $(this).one('load error', function() { if (--left === 0) cb(); });
        });
    }
    ...
    $(window).on('pagehide', function() {
        ...
        $.each(window._swipers, function(_, sw) {
            try { sw && !sw.destroyed && sw.destroy(true, true); } catch (_) {}
        });
    });
    ...
    $(document).on('visibilitychange', function() {
        ...
        if (!s) return;
        if (document.hidden) {
            try { s.logos && s.logos.autoplay && s.logos.autoplay.stop(); } catch (_) {}
        } else {
            try {
    ...
    function isIOSSafari() {
        ...
        var iOS = /iP(hone|ad|od)/.test(navigator.platform) || /Macintosh/.test(ua) && 'ontouchend' in document;
    ...
    $(document).on('wpcf7mailsent', function(event) {
        ...
        if (window.CURRENT_RESOURCE_ID && sentResourceId && String(sentResourceId) !== String(window.CURRENT_RESOURCE_ID)) return;

        // получаем URL
        var downloadUrl =
            $block.data('pending-download-url') ||
            $block.find('.strategic_card_content .btn_group .download-btn').data('download-url') ||
            $block.find('.strategic_card_content .btn_group .download-btn').attr('href');
        ...
        var opened = null;
        try { opened = window.open(downloadUrl, '_blank', 'noopener'); } catch (e) {}
        if (opened) return;
        ...
        try {
            ...
            ifr.onload = function() { setTimeout(function() { ifr.remove(); }, 10000); };
            ifr.src = downloadUrl;
...
$(document).ready(function() {
    ...
    $(document).on('touchstart', SEL, function(e) {
        ...
        var t = setTimeout(function() { $a.data('longpress', true); }, LONG_MS);
        $a.data('lpTimer', t);
    });
    ...
    (function() {
        ...
        document.addEventListener('touchstart', function(e) {
            ...
            if (!a) return;
            long = false;
            timer = setTimeout(function() { long = true; }, 450);
        }, { passive: true });
        ...
        document.addEventListener('touchcancel', function() { clearTimeout(timer); });
    })();
